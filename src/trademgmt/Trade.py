...
class Trade:
  def __init__(self, tradingSymbol = None, strategy = ""):
    ...
    self.slWhiskerHit = False # True if SL was fired by a tick not confirmed by 1-min candle high/low
    ...
    self.tag = None # Strategy-assigned leg identifier (e.g. CE_ATM, PE_OTM1) for shadow matching
    self.shadowDeviationStatus = {} # keyed by DeviationType, value is DeviationStatus
  ...
  def __str__(self):
    return "ID=" + str(self.tradeID) + ", state=" + self.tradeState + ", symbol=" + self.tradingSymbol \
      ...
      + ", MarketOrder=" + str(self.placeMarketOrder) +", productType=" + self.productType + ", reqEntry=" + str(self.requestedEntry) \
